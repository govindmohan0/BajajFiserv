...
const AdminDashboard = () => {
  ...
  const handleMLAlgorithm = (userId) => {
    console.log(`Running ML algorithm for user ID: ${userId}`);
    // Add your ML algorithm call here
  };
        ...
        if (response.ok) {
          ...
          return response.json().then((errorData) => {
            ...
            setUsers((prevUsers) => {
              ...
              console.log('Reverting optimistic update, adding user back:', revertedUser);
      ...
      <div className="overflow-x-auto bg-white shadow-md rounded-lg">
        <table className="min-w-full bg-white">
          ...
          <tbody className="text-gray-700">
            {users.map((user) => (
              <tr key={user.id || user._id} className="border-b border-gray-200 hover:bg-gray-100">
